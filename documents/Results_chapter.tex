...
In contrast, Support Vector Machines paired with minority oversampling demonstrated more stable boundary generation. The selected \textbf{SMOTE + SVM} pipeline achieved a cross-validation F1-score of 0.3142, a recall of 44.29\%, an accuracy of 74.19\%, and an ROC-AUC of 0.6413. By applying synthetic oversampling, the SVM algorithm constructed a wider margin separating hyperplane that did not collapse under class skewness. Table~\ref{tab:cv_model_performance} summarizes the 5-fold cross-validation performance metrics for a selection of representative classifier--resampler configurations.

\begin{table}[htbp]
    \centering
    \caption{5-Fold Cross-Validation Performance Comparison (Training Set, $N_{\text{train}} = 256$)}
    \label{tab:cv_model_performance}
    \resizebox{\textwidth}{!}{%
    \begin{tabular}{lcccccc}
        \toprule
        \textbf{Resampling Strategy} & \textbf{Classifier} & \textbf{Accuracy} & \textbf{Precision} & \textbf{Recall} & \textbf{F1-Score} & \textbf{ROC-AUC} \\
        \midrule
        ...
        \textbf{SMOTE-ENN} & Gradient Boosting & 0.7888 & 0.3234 & 0.4714 & 0.3779 & 0.7347 \\
        \textbf{SMOTE}     & Support Vector Machine & 0.7419 & 0.2572 & 0.4429 & 0.3142 & 0.6413 \\
        \textbf{Original}  & Support Vector Machine & 0.7732 & 0.3004 & 0.4762 & 0.3538 & 0.6784 \\
        \textbf{Original}  & Gradient Boosting & 0.8790 & 0.7000 & 0.2048 & 0.3076 & 0.6961 \\
        \bottomrule
    \end{tabular}%
    }
\end{table}

Because the primary design objective of an educational Early Warning System is to identify at-risk students for proactive counselor check-ins, optimizing for a higher and more balanced recall is critical. Consequently, the SMOTE + SVM pipeline was selected as the final deployed EWS prediction model.
...
While the SMOTE + SVM model achieves a high recall of 90.91\% on the holdout test set (identifying 10 out of 11 actual at-risk students), its low specificity (30.43\%, corresponding to a False Positive Rate of 69.57\%) introduces a significant operational trade-off that must be examined for real-world deployment. In the holdout test set of 80 students, the model flags a total of 58 students (10 True Positives and 48 False Positives) as at-risk, meaning that 72.5\% of the entire student body is flagged for intervention.

In a practical counseling workflow, this high false alert rate creates several challenges:
\begin{itemize}
    \item \textbf{Resource-Constrained Workload}: In an average Cambodian secondary school with 1,000 students, a 72.5\% alert rate translates to 725 flagged students. Given that most schools operate with very limited counseling staff (often a single teacher acting as a part-time counselor), conducting deep-dive individual follow-ups with three-quarters of the student body is logistically unfeasible.
    \item \textbf{Alert Fatigue}: Continuous false alerts can lead to alert fatigue, causing teachers and counselors to lose trust in the early warning system's recommendations and potentially overlook genuinely at-risk students.
    \item \textbf{Intervention Dilution}: Diluting limited intervention resources (such as NGO scholarships, tutoring, or direct home visits) across a large group of false positives reduces the support intensity available for the 10\% of students who are genuinely on the verge of dropping out.
\end{itemize}

To mitigate these operational costs and make the early warning system actionable in school counseling workflows, we propose two practical deployment strategies:

\begin{enumerate}
    \item \textbf{Dynamic Threshold Adjustment}: Counselors can dynamically adjust the probability threshold $\tau$ depending on their available counseling bandwidth. As shown in the threshold tuning curve (Fig.~\ref{fig:threshold_tuning}), setting $\tau = 0.50$ on the validation set reduces the false alert rate, yielding a validation Recall of 62.50\% and a Precision of 21.74\%. If bandwidth is extremely limited, raising the threshold to $\tau = 0.55$ achieves a validation Recall of 62.50\% and a Precision of 31.25\%. This significantly improves specificity (specificity increases to over 80\%) and reduces the number of flagged students to a manageable 15--20\% of the student population, representing a much more targeted and efficient use of resources.
    \item \textbf{Tiered Intervention Protocol}: Instead of treating the classification output as binary, the dashboard can display the raw risk probabilities to implement a tiered support structure:
        \begin{itemize}
            \item \textbf{High-Probability Tier ($\ge 0.70$)}: Reserved for high-intensity, resource-heavy interventions (e.g., direct NGO scholarship applications, family agricultural aid, and immediate home visits).
            \item \textbf{Medium-Probability Tier ($0.30 \le \text{Risk} < 0.70$)}: Standard interventions (e.g., peer-mentoring groups, attendance tracking cards, and monthly check-ins).
            \item \textbf{Low-Probability Tier ($< 0.30$)}: No direct intervention, standard classroom monitoring.
        \end{itemize}
\end{enumerate}
This analysis demonstrates that the early warning system is not a rigid binary switch, but a flexible screening tool that can be optimized to balance sensitivity and counselor workload according to the specific resources of each educational institution.
