...
\paragraph{Why SMOTE + SVM was deployed despite lower CV F1.}
The SMOTE-ENN + LightGBM configuration achieves the best cross-validation F1 (0.4141), which makes it the top-ranked model by standard benchmark criteria. However, the cross-validation F1 is computed at the \textit{default threshold} $\tau = 0.50$. For an EWS, the deployment decision must account for two additional factors:

\begin{enumerate}
    \item \textbf{Recall is the priority metric, not F1.} At $\tau = 0.50$, SMOTE-ENN + LightGBM achieves only 59.05\% recall --- meaning it misses approximately 41\% of at-risk students in cross-validation. The SVM pipeline, after threshold tuning to $\tau = 0.30$ on the validation set, achieves \textbf{90.91\% recall} on the holdout test (Section~\ref{sec:holdout_evaluation_results}). No threshold-tuned evaluation was conducted for LightGBM in this study.

    \item \textbf{Probability calibration and deployment stability.} The SVM with \texttt{probability=True} uses Platt scaling to produce well-calibrated probability outputs, which directly enables the continuous risk-score display in the EWS dashboard and the tiered intervention protocol (Section~\ref{subsec:operational_workload}). LightGBM probability outputs are less calibrated by default and would require an additional calibration step (e.g., isotonic regression) before threshold tuning could be applied reliably on a dataset of this size ($N = 400$).
\end{enumerate}

The deployed pipeline is therefore:
\begin{center}
...
This pipeline was selected \textit{not} because SVM is the best model by CV F1, but because SVM + threshold tuning yields the highest recall on unseen data while providing calibrated probability scores needed for the dashboard. The SMOTE-ENN + LightGBM configuration is retained as the benchmark reference and a recommended direction for future work when a larger dataset is available for robust calibration.
