...
\paragraph{Study limitation: threshold tuning was applied only to SVM.}

In principle, the objectively best cross-validation model --- \textbf{SMOTE-ENN + LightGBM} (F1 = 0.4141, ROC-AUC = 0.7559) --- should be the deployed model. Selecting a weaker CV model for deployment requires a clear, principled justification. The critical limitation of this study is that \textit{decision threshold tuning was only performed on the SMOTE + SVM pipeline}, not on LightGBM or any other model. This means the 59.05\% recall of LightGBM (Table~\ref{tab:cv_model_performance}) is measured at the default $\tau = 0.50$, which is not the recall-optimised operating point.

Had threshold tuning been applied to SMOTE-ENN + LightGBM under the same conditions (sweeping $\tau$ on the validation set and selecting for maximum recall), it is expected --- given its superior ROC-AUC of 0.7559 vs.\ 0.6413 for SVM --- that LightGBM would achieve a \textit{higher} recall at an equivalent false-alert rate. Therefore, \textbf{SMOTE-ENN + LightGBM is the recommended model for future deployment} when proper threshold calibration and holdout evaluation can be conducted.

For the scope of this study, the deployed pipeline is the \textbf{SMOTE + SVM} model, selected because:
\begin{itemize}
    \item Threshold tuning and holdout evaluation were fully executed only for this pipeline (Section~\ref{sec:threshold_tuning_results} and Section~\ref{sec:holdout_evaluation_results}).
    \item The SVM with \texttt{probability=True} uses Platt scaling, producing well-calibrated probability outputs suitable for the EWS dashboard's tiered risk display.
    \item After hyperparameter tuning via 5-fold \texttt{RandomizedSearchCV} ($n_{\text{iter}} = 20$, search space: $C \in \{0.1, 1, 10, 100\}$, $\gamma \in \{\texttt{scale}, \texttt{auto}, 0.01, 0.1, 1\}$), the best parameters were $C = 0.1$ and $\gamma = 0.01$, reflecting a soft-margin, wide-kernel SVM that generalises conservatively on the small dataset.
\end{itemize}

The final deployed pipeline is:
\begin{center}
    \texttt{StandardScaler} $\rightarrow$ \texttt{SMOTE(random\_state=42)} $\rightarrow$ \texttt{SVC(C=0.1, $\gamma$=0.01, kernel=`rbf', class\_weight=`balanced', probability=True)}
\end{center}

\begin{quote}
\textit{Limitation: The fair comparison between threshold-tuned SMOTE-ENN + LightGBM and threshold-tuned SMOTE + SVM on the holdout set was not conducted in this study. Future work should apply threshold tuning to all candidate models and select the deployed model based on recall-at-threshold, not default-threshold F1.}
\end{quote}
