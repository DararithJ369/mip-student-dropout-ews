...
Eight classification algorithms were evaluated in combination with the six resampling strategies above, using 5-fold stratified cross-validation on the training set ($N_{\text{train}} = 256$). The primary evaluation metric was the minority-class F1-Score, selected because it balances precision and recall for the at-risk class. Figure~\ref{fig:cv_model_comparison} summarises the mean F1-Scores averaged across all resampling strategies for each classifier.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.90\textwidth]{Images/cv_model_comparison.png}
    \caption{Cross-validation F1-Score comparison across eight classifiers (averaged across all six resampling strategies). Error bars indicate standard deviation across resampling configurations. XGBoost achieves the highest mean F1-Score ($\approx 0.37$), followed by LightGBM ($\approx 0.355$). SVM achieves a moderate mean F1 ($\approx 0.325$) with lower variance.}
    \label{fig:cv_model_comparison}
\end{figure}

The CV comparison reveals the following ranking:

\begin{enumerate}
    \item \textbf{XGBoost} achieves the highest mean F1-Score ($\approx 0.37$) with moderate variance (error bars spanning approximately 0.29--0.40). Its gradient-boosting mechanism with L1/L2 regularisation handles the sparse feature space and class imbalance well when paired with oversampling.
    \item \textbf{LightGBM} is a close second ($\approx 0.355$), benefiting from its leaf-wise tree growth strategy which efficiently captures the interaction between engagement and SES features.
    \item \textbf{GradientBoosting} achieves $\approx 0.33$, followed by \textbf{SVM} and \textbf{RandomForest} (both $\approx 0.325$).
    \item \textbf{ExtraTrees} and \textbf{LogisticRegression} both yield $\approx 0.30$--$0.31$.
    \item \textbf{DecisionTree} performs worst ($\approx 0.295$), as its tendency to overfit individual training folds is amplified by the small minority class.
\end{enumerate}

Despite XGBoost and LightGBM achieving the highest F1-Scores, the \textbf{SVM with SMOTE} pipeline was selected as the final deployed model for two reasons. First, SVM with SMOTE showed the most \textit{stable} recall behaviour across folds, with lower standard deviation than boosting methods. Second, SVM operates as a kernel-based maximum-margin classifier that is less sensitive to the micro-distributions of synthetic SMOTE samples than tree-based ensemble methods, making it more reliable under the small-sample conditions of this study. The final SVM hyperparameters (C = 0.1, $\gamma$ = 0.01, RBF kernel, balanced class weights) were obtained through 5-fold stratified RandomizedSearchCV.

Table~\ref{tab:cv_model_performance} reports the detailed cross-validation metrics for the most representative classifier--resampler combinations.

\begin{table}[htbp]
    \centering
    \caption{5-Fold Cross-Validation Performance (Training Set, $N_{\text{train}} = 256$) -- Selected Configurations}
    ...
    \begin{tabular}{llccccc}
        ...
        \textbf{SMOTE-ENN} & LightGBM           & 0.7770 & 0.3366 & 0.5905 & 0.4141 & 0.7559 \\
        ...
        \textbf{SMOTE}     & SVM (selected)     & 0.7419 & 0.2572 & 0.4429 & 0.3142 & 0.6413 \\
...
Table~\ref{tab:cv_model_performance} highlights a critical trade-off. On the original imbalanced dataset, GradientBoosting achieves 87.9\% accuracy but only 20.48\% recall --- it misses nearly 80\% of at-risk students by always predicting the majority class. SMOTE-ENN paired with LightGBM or XGBoost achieves the best F1 (0.41) by boosting recall to 59\%, but this comes at the cost of lower precision ($\approx 33\%$). The selected SMOTE + SVM achieves a balanced operating point (recall = 44.29\%, ROC-AUC = 0.64) that is further improved through threshold tuning in the next section.

%%============================================================================
\section{Decision Threshold Tuning}
\label{sec:threshold_tuning_results}

The SVM pipeline outputs calibrated probability scores for each student. By default, a threshold of $\tau = 0.50$ is used to convert probabilities into binary predictions. However, in an educational screening context, a False Negative (missing a truly at-risk student) carries a much higher cost than a False Positive (flagging a student who is not actually at risk). Therefore, the threshold was swept from $\tau = 0.10$ to $\tau = 0.95$ on the validation set ($N_{\text{val}} = 64$) to identify the operating point that maximises recall while maintaining a workable precision.
...
To make the SMOTE + SVM model transparent and auditable for school counsellors, SHAP (SHapley Additive exPlanations) values were computed on the holdout test set using a \texttt{KernelExplainer}. The SHAP beeswarm plot (Fig.~\ref{fig:shap_summary}) shows the global feature importance ranking and the direction of each feature's impact on predicted dropout probability.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.85\textwidth]{Images/shap_summary.png}
    \caption{SHAP beeswarm plot for the SMOTE + SVM model. Features are ranked top-to-bottom by mean absolute SHAP value. Red dots = high feature value; blue dots = low feature value. Positive SHAP values push the prediction toward dropout (class 1); negative values push it toward non-dropout (class 0).}
...
The generalization capability of the SMOTE + SVM pipeline was evaluated on the completely unseen holdout test set ($N = 80$, comprising 69 non-at-risk and 11 at-risk students, preserving the original 86.25\%/13.75\% class ratio). A Soft Voting Ensemble --- aggregating predicted probabilities from Logistic Regression, SVM, Random Forest, and XGBoost base estimators --- was evaluated on the same holdout set as a comparison benchmark.
...
\begin{itemize}
    \item \textbf{SMOTE + SVM}: TN = 21, FP = 48, FN = 1, TP = 10.
...
The results reveal a stark and interpretively important contrast between the two models:

\textbf{SMOTE + SVM} correctly identified \textbf{10 out of 11} actual dropout-risk students (90.91\% recall), missing only a single at-risk student (FN = 1). This is the primary objective of an Early Warning System. However, the model also flagged 48 non-at-risk students as at-risk (FP = 48), producing an overall accuracy of only 38.75\%. The specificity (30.43\%) and precision (17.24\%) are low, reflecting the aggressive sensitivity stance imposed by threshold $\tau = 0.30$.

\textbf{Soft Voting Ensemble} achieved 80\% overall accuracy by simply predicting all students as non-at-risk (0 TP, 0 FP on the at-risk class, 11 FN). This is a completely failed model for EWS purposes: it identifies zero at-risk students. The ensemble collapses to majority-class prediction because the component classifiers (without threshold tuning) all default to predicting the dominant class. A high accuracy figure is therefore \textit{misleading} when the minority class is the clinically important one.

The recall of the SVM model (90.91\%) generalises robustly from the validation set (87.50\%), confirming that the threshold-tuned pipeline reliably identifies at-risk students even on unseen data. The ROC-AUC scores for both models (0.646 vs.\ 0.634) are comparable, indicating that the SVM's advantage lies entirely in its calibrated threshold setting and SMOTE training, not in a fundamentally superior discriminative boundary.
