...
\begin{table}[htbp]
    ...
    \begin{tabular}{clr}
        \toprule
        \textbf{Rank} & \textbf{Survey Feature} & \textbf{Cram\'{e}r's V} \\
        \midrule
        1  & Weekly attendance days (\texttt{attendance})        & 0.2656 \\
        2  & Absence frequency (\texttt{absence})                & 0.2469 \\
        3  & Subjective family income (\texttt{family\_income})  & 0.2178 \\
        4  & Monthly score average (\texttt{monthly\_average})   & 0.1427 \\
        5  & Living arrangements (\texttt{living\_with})         & 0.1047 \\
        6  & Primary transport mode (\texttt{transport})         & 0.0994 \\
        7  & Distance to school (\texttt{distance})              & 0.0575 \\
        8  & Student gender (\texttt{gender})                    & 0.0317 \\
        9  & Parental education (\texttt{parental\_education})   & 0.0000 \\
        10 & Works to support family (\texttt{work\_support})    & 0.0000 \\
        11 & External NGO/scholarship aid (\texttt{external\_support}) & 0.0000 \\
        \bottomrule
    \end{tabular}
\end{table}

The association analysis reveals that school disengagement metrics (attendance quality and absence frequency) and household socioeconomic status (family income) are the three strongest individual statistical predictors of dropout intention. Notably, features such as parental education, working to support family, and access to external aid all receive bias-corrected Cram\'{e}r's V values of 0.0000, indicating that while these variables are contextually important (and indeed validated by the literature), they carry no individual discriminative power in isolation and must be combined into composite engineered features to be effective in a predictive model.
...
\begin{longtable}{|p{2.8cm}|p{5.0cm}|p{7.8cm}|}
    ...
    $WRS = 0.3 \cdot x_{\text{absence}} + 0.1 \cdot x_{\text{work\_support}} + 0.2 \cdot x_{\text{distance}} + 0.4 \cdot y_{\text{thought\_dropout}}$ &
